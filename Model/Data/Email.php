...
class Email
{
    ...
    {
        $this->storeName  = $this->_scopeConfig->getValue(
            'trans_email/ident_sales/name',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
        $this->storeEmail = $this->_scopeConfig->getValue(
    ...
    private function getEmailData($order)
    {
        ...
        switch ($paymentType) {
            case 'bill_payment_tesco_lotus':
                ...
                $emailData->addData(
                    ['barcode' => $this->_helper->convertTescoSVGCodeToHTML(
                        $payment->getAdditionalInformation('barcode')
                    )]
                );
            ...
            case 'paynow':
                ...
                $emailData->addData(
                    ['barcode' => "<img src= '".$charge->source['scannable_code']['image']['download_uri']."'/>"]
                );
                $emailData->addData(
                    ['banksUrl' => $this->_assetRepo->getUrl('Omise_Payment::images/paynow_supportedbanks.png')]
                );
            ...
            case 'promptpay':
                ...
                date_default_timezone_set("Asia/Bangkok");
                $emailData->addData(
                    ['barcode' => $this->_storeManager->getStore()->getBaseUrl(
                        \Magento\Framework\UrlInterface::URL_TYPE_WEB
                    )."omise/payment/complete/orderId/".$order->getIncrementId()]
                );
        ...
        $emailData->addData(
            ['amount' => number_format($paymentData['amount_ordered'], 2) . ' ' .
    ...
    {
        $timestamp = time() > strtotime('tomorrow - 1seconds') ?
            strtotime('tomorrow + 1day - 1second') : strtotime('tomorrow - 1seconds');
